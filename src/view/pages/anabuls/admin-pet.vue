        ...
        <template v-slot:body>
          <div>
            <b-table-simple>
              ...
              <b-tbody>
                <b-tr v-for="(item, index) in adminPet" :key="item.id">
...
export default {
  ...
  methods: {
    ...
    fetchRequest(page = 1) {
      this.$api
        .get(
          `admin/all?perPage=${this.perPage}&page=${page}&search=${this.search}&sortBy=${this.sortBy}&orderBy=${this.orderBy}`
        )
        .then(res => {
          this.adminPet = res.data.data.data ? res.data.data.data : [];
          // console.log(this.adminPet);
  ...
  computed: {
    countSection() {
      ...
      if (this.perPage === 1) {
        ...
        if (this.adminPet.length === this.perPage) {
          meta.endSection = this.page * this.perPage;
          meta.startSection = meta.endSection - (this.perPage - 1);
        } else {
          meta.endSection =
            this.page * this.perPage - (this.perPage - this.adminPet.length);
          meta.startSection = meta.endSection - (this.adminPet.length - 1);
        }
      }
      return meta;
    }
