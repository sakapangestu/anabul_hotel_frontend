        ...
        <div class="col-md-5 ml-2 mb-5 mt-4 pl-1 pr-1 search-section">
          <div class="input-group">
            ...
            <input
              ...
              placeholder="Search name regulasi hewan"
              v-model="search"
              @input="fetchRegulation()"
            />
          </div>
        </div>
        ...
        <template v-slot:body>
          <div>
            <b-table-simple>
              ...
              <b-tbody>
                <b-tr v-for="(item, index) in regulasi" :key="item.id">
            ...
            <nav aria-label="..." class="float-right">
              <ul class="pagination">
                ...
                <li
                  ...
                  :key="pg.id"
                >
                  <a class="page-link" href="#" @click="fetchRegulation(pg)">{{
                    pg
                  }}</a>
                </li>
...
export default {
  ...
  methods: {
    ...
    //     });
    // },
    fetchRegulation(page = 1) {
      this.$api
        .get(
          `regulation/all?perPage=${this.perPage}&page=${page}&search=${this.search}&sortBy=${this.sortBy}&orderBy=${this.orderBy}`
        )
        .then(res => {
          this.regulasi = res.data.data.data ? res.data.data.data : [];
          console.log(this.regulasi);
    ...
    handleSubmit() {
      ...
      if (this.isEdit) {
        ...
        this.$api
          .put("regulation/update", this.addForm)
          .then(res => {
            if (res.status === 200) {
              this.hideModal();
              this.fetchRegulation();
  ...
  computed: {
    countSection() {
      ...
      if (this.perPage === 1) {
        ...
        if (this.regulasi.length === this.perPage) {
          meta.endSection = this.page * this.perPage;
          meta.startSection = meta.endSection - (this.perPage - 1);
        } else {
          meta.endSection =
            this.page * this.perPage - (this.perPage - this.regulasi.length);
          meta.startSection = meta.endSection - (this.regulasi.length - 1);
        }
      }
      return meta;
    }
