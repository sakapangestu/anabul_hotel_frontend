        ...
        <div class="col-md-5 ml-2 mb-5 mt-4 pl-1 pr-1 search-section">
          <div class="input-group">
            ...
            <input
              ...
              placeholder="Search name spesies baru"
              v-model="search"
              @input="fetchNewSpesies()"
            />
          </div>
        </div>
        ...
        <template v-slot:body>
          <div>
            <b-table-simple>
              ...
              <b-tbody>
                <b-tr v-for="(item, index) in newSpesies" :key="item.id">
                  ...
                  <b-td>
                    <b-badge variant="success" v-if="item.status === 'Diterima'"
                      >Diterima</b-badge
                    >
                    <b-badge variant="warning" v-if="item.status === 'Diproses'"
                      >Diproses</b-badge
                    >
                    <b-badge variant="danger" v-if="item.status === 'Ditolak'"
                      >Ditolak</b-badge
                    >
                  </b-td>
            ...
            <nav aria-label="..." class="float-right">
              <ul class="pagination">
                ...
                <li
                  ...
                  :key="pg.id"
                >
                  <a class="page-link" href="#" @click="fetchNewSpesies(pg)">{{
                    pg
                  }}</a>
                </li>
...
export default {
  ...
  methods: {
    ...
    // },
    showModal() {
      this.modalTitle = "Tambah Spesies Baru";
    ...
    fetchNewSpesies(page = 1) {
      this.$api
        .get(
          `tempSpecies/all?hotel_id=${this.hotelId}&perPage=${this.perPage}&page=${page}&search=${this.search}&sortBy=${this.sortBy}&orderBy=${this.orderBy}`
        )
        .then(res => {
          this.newSpesies = res.data.data.data ? res.data.data.data : [];
          console.log(this.newSpesies);
    ...
    onDelete(id) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.$api
            .delete(`tempSpecies/delete/${id}`)
            .then(res => {
              if (res.status === 200) {
                this.fetchNewSpesies();
                Swal.fire({
                  icon: "warning",
    ...
    handleSubmit() {
      ...
      if (this.isEdit) {
        ...
        this.$api
          .put("tempSpecies/update", this.addForm)
          .then(res => {
            if (res.status === 200) {
              this.hideModal();
              this.fetchNewSpesies();
        ...
        this.addForm.hotel_id = this.hotelId;
        this.$api
          .post("tempSpecies/add", this.addForm)
          .then(res => {
            if (res.status === 201) {
              this.hideModal();
              this.fetchNewSpesies();
  ...
  computed: {
    countSection() {
      ...
      if (this.perPage === 1) {
        ...
        if (this.newSpesies.length === this.perPage) {
          meta.endSection = this.page * this.perPage;
          meta.startSection = meta.endSection - (this.perPage - 1);
        } else {
          meta.endSection =
            this.page * this.perPage - (this.perPage - this.newSpesies.length);
          meta.startSection = meta.endSection - (this.newSpesies.length - 1);
        }
      }
      return meta;
    }
